...
async def clone_page_cb(client, CallbackQuery, _):
    ...
    await CallbackQuery.edit_message_media(
        ...
        reply_markup=InlineKeyboardMarkup(
            [
                [InlineKeyboardButton(text="⌯ ʙᴀᴄᴋ ⌯", callback_data="settingsback_helper", style=alone_style)]
            ]
        )
    )
...
async def gib_repo_callback(_, callback_query):
    ...
    await callback_query.edit_message_media(
        ...
        reply_markup=InlineKeyboardMarkup(
            [
                [
                    InlineKeyboardButton(text="• ʙᴀᴄᴋ •", callback_data="settingsback_helper", style=group_style),
                    InlineKeyboardButton(text="• ᴄʟᴏsᴇ •", callback_data="close", style=group_style)
                ]
            ]
        ),
    )
...
async def del_back_playlist(client, CallbackQuery, _):
    ...
    if command == "UpVote":
        ...
        if get_upvotes >= upvote:
            ...
            alone_style = random.choice(STYLES)
            upl = InlineKeyboardMarkup([[InlineKeyboardButton(text=f"👍 {get_upvotes}", callback_data=f"ADMIN  UpVote|{chat_id}_{counter}", style=alone_style)]])
